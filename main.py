#!/usr/bin/env python3
"""
main.py — M-D E-Commerce Web Scraper

Interactive and argument-driven CLI for e-commerce data collection.
Supports multiple platforms: Amazon, Noon, AliExpress, Jumia, and eBay.

Core Capabilities:
    - Search and scrape products across multiple e-commerce sites
    - Store data in SQLite database
    - Export results as CSV, Excel, or JSON
    - Analyze product data with statistics and insights
    - Interactive guided mode or command-line arguments
...
Usage:
    ...
    python main.py scrape amazon 5                     # 5 pages
    python main.py scrape noon 10 -c Electronics       # With category
    python main.py scrape all 3 -q laptop              # All sites with query

    # View data
    python main.py analyze                             # Full analysis
    python main.py stats                               # Quick stats
    python main.py sites                               # List all sites

    # Export data
    python main.py export csv                          # Export as CSV
    python main.py export all                          # Export all formats

Architecture:
    Input → Scraper Factory → Individual Scrapers → Data Cleaner → 
    Storage → Optional Export → Analysis
...
from src.scrapers.factory import (
    create_scraper, list_sites, site_keys, REGISTRY, add_custom_site,
...
def run_scrape(
    ...
    """
    Execute a complete scrape-clean-export pipeline.
    
    This is the main orchestration function that:
    1. Creates scraper instances for each source
    2. Executes scrapes and tracks results
    3. Cleans collected data
    4. Stores in SQLite database
    5. Exports to requested format(s)
    
    Args:
        sources: List of site keys to scrape (["amazon", "noon"], etc)
        max_pages: How many pages per site to scrape
        category: Optional category filter
        query: Optional search keyword
        fmt: Export format (csv, excel, json, all)
        output_stem: Custom filename prefix for exports
    """
    db       = SQLiteStorage(settings.db_path)
    exporter = DataExporter(str(settings.processed_dir))
    all_prods = []

    # Create scraper configuration with user settings
    cfg = ScraperConfig(
        max_pages   = max_pages,
...
def run_analyze() -> None:
    """
    Perform comprehensive analysis of all stored products.
    
    Generates statistics including:
    - Price distribution (mean, median, quantiles, std dev)
    - Product breakdown by source and category
    - Top-rated products
    - Best discounts
    ...
    # Reconstruct Product objects from database
...
def run_stats() -> None:
    """
    Display quick overview statistics of the database.
    Shows: total products, sources, categories, average price, etc.
    """
    db    = SQLiteStorage(settings.db_path)
    stats = db.get_stats()
...
def run_export(fmt: str) -> None:
    """
    Export all products in database to file(s).
    
    Args:
        fmt: Export format - csv, excel, json, or all
    """
    db   = SQLiteStorage(settings.db_path)
    rows = db.get_all_dicts()
    ...
    # Reconstruct Product objects from database rows
    products = []
    for r in rows:
        try:
...
def interactive_mode() -> None:
    """
    Main interactive session - presents menu and guides user through options.
    Allows users to scrape, analyze, export, or view stats without command-line args.
    """
    while True:
...
def _interactive_clear() -> None:
    """Interactive confirmation to clear all database data."""
...
def _interactive_scrape() -> None:
    ...
    Guides user through selecting sites, categories, pages, and export format.
    """
...
def print_summary_box(products) -> None:
    """
    Print a formatted summary box after scrape completes.
    Shows total products, sources, categories, prices, and discount stats.
    """
    if not products:
        return
...
def build_parser() -> argparse.ArgumentParser:
    """
    Build and return argument parser for CLI mode.
    Handles commands: scrape, analyze, export, stats, sites
    """
...
def cmd_sites() -> None:
    """Display all available scraping sites with metadata."""
...
def main() -> None:
    ...
    Handles startup, mode detection (interactive vs CLI), and main command dispatch.
    """
    # Load custom sites that were previously saved by users
    load_saved_custom_sites()
